...
void main() {
  group('convertToIndexItem', () {
    ...
    final createdPredicate = IriTerm('https://schema.org/dateCreated');
    final modifiedPredicate = IriTerm('https://schema.org/dateModified');
    final keywordsPredicate = IriTerm('https://schema.org/keywords');

    test('converts resource to index item with filtered properties', () {
      // Create a test resource with multiple properties
      final resourceSubject = IriTerm('https://example.org/notes/test-note');
      final resourceGraph = RdfGraph(triples: [
        Triple(resourceSubject, titlePredicate, LiteralTerm.string('Test Note')),
        Triple(resourceSubject, contentPredicate, LiteralTerm.string('This is the full content')),
        Triple(resourceSubject, createdPredicate, LiteralTerm.string('2023-01-01T10:00:00Z')),
        Triple(resourceSubject, modifiedPredicate, LiteralTerm.string('2023-01-02T15:30:00Z')),
        Triple(resourceSubject, keywordsPredicate, LiteralTerm.string('work')),
        Triple(resourceSubject, keywordsPredicate, LiteralTerm.string('important')),
      ]);
      ...
      expect(indexTriples.any((t) =>
        t.predicate == IdxVocab.resource && t.object == resourceSubject),
        isTrue, reason: 'Should include idx:resource reference');

      // Should have filtered properties
      expect(indexTriples.any((t) =>
        t.predicate == titlePredicate),
        isTrue, reason: 'Should include title');

      expect(indexTriples.any((t) =>
        t.predicate == createdPredicate),
        isTrue, reason: 'Should include creation date');

      expect(indexTriples.any((t) =>
        t.predicate == keywordsPredicate),
        isTrue, reason: 'Should include keywords');

      // Should NOT have the content property (filtered out)
      expect(indexTriples.any((t) =>
        t.predicate == contentPredicate),
        isFalse, reason: 'Should filter out content property');
    ...
    test('handles resource with minimal properties', () {
      final resourceSubject = IriTerm('https://example.org/notes/minimal');
      final resourceGraph = RdfGraph(triples: [
        Triple(resourceSubject, titlePredicate, LiteralTerm.string('Minimal Note')),
        Triple(resourceSubject, createdPredicate, LiteralTerm.string('2023-01-01T10:00:00Z')),
      ]);
      ...
      expect(indexTriples.any((t) =>
        t.predicate == titlePredicate),
        isTrue, reason: 'Should include available title');

      expect(indexTriples.any((t) =>
        t.predicate == createdPredicate),
        isTrue, reason: 'Should include available creation date');

      // Should include idx:resource reference
      expect(indexTriples.any((t) =>
        t.predicate == IdxVocab.resource),
        isTrue, reason: 'Should include idx:resource reference');

      // Should not include missing properties (keywords)
      expect(indexTriples.any((t) =>
        t.predicate == keywordsPredicate),
        isFalse, reason: 'Should not include missing keywords property');
    ...
    test('always includes idx:resource reference', () {
      ...
      final resourceGraph = RdfGraph(triples: [
        ...
        Triple(resourceSubject, contentPredicate, LiteralTerm.string('Content')),
      ]);
      ...
      expect(indexTriples.any((t) =>
        t.predicate == IdxVocab.resource && t.object == resourceSubject),
        isTrue, reason: 'Should automatically add idx:resource reference');

      // Should include the title property
      expect(indexTriples.any((t) =>
        t.predicate == titlePredicate),
        isTrue, reason: 'Should include requested title property');

      // Should not include content (not requested)
      expect(indexTriples.any((t) =>
        t.predicate == contentPredicate),
        isFalse, reason: 'Should not include unrequested content property');
    });

    test('handles empty property set', () {
      final resourceSubject = IriTerm('https://example.org/notes/empty');
      final resourceGraph = RdfGraph(triples: [
