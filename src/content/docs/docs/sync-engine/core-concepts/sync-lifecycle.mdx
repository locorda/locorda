...
<Aside type="note">
**Quick orientation:** `initLocorda()` returns a `locorda` instance. From it, you obtain an `ObjectSyncEngine` and pass it to your repository. During repository initialization, you call `syncEngine.hydrateWithCallbacks()` to register your `onUpdate` / `onDelete` callbacks — these are what keep your local database in sync. When your UI then calls `repository.save(task)`, the repository forwards it to `syncEngine.save<Task>(task)` internally.

See [Getting Started](/docs/sync-engine/guides/getting-started) and [Repository Pattern](/docs/sync-engine/guides/repository-pattern) for the full setup.
</Aside>
...
<Aside type="note">
**Planned triggers:** Network connectivity change and app resume are not yet implemented. For now, trigger sync manually after reconnection via `syncManager.sync(trigger: SyncTrigger.manual)` or enable periodic auto-sync.
</Aside>
