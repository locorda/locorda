...
The repository pattern works with any state management solution:
...
class TaskNotifier extends ChangeNotifier {
  ...
  TaskNotifier(this._repo) {
    _subscription = _repo.watchAll().listen((_) {
      notifyListeners();
    });
  }
  
  List<Task> get tasks => _repo.getAll();
}
```
...
final taskRepositoryProvider = Provider<TaskRepository>((ref) {
  return TaskRepository.create(ref.watch(syncEngineProvider));
});
...
class TaskBloc extends Bloc<TaskEvent, TaskState> {
  ...
  TaskBloc(this._repository) : super(TaskInitial()) {
    _subscription = _repository.watchAll().listen((tasks) {
...
- Don't forget to call `_notifyListeners()` in your callbacks
