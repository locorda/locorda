...
- **dataChange** - Sync triggered after local data changes
- **connectionRestore** - Sync triggered after network connectivity is restored
...
<Aside type="tip">
**Manual retry is always available**: Users can trigger manual sync via `syncManager.sync()` regardless of automatic retry state. See [Monitoring Sync State](#monitoring-sync-state) for the complete API.
</Aside>
